# Domain name for monitoring dashboard ingress (from GitHub Variable: DOMAIN_NAME)
variable "domain_name" {
  type = string
}

# GitHub username for Git source reference (auto from github.repository_owner)
variable "github_username" {
  type = string
}

# GitHub repository name for values files source (auto from github.event.repository.name)
variable "github_repo" {
  description = "GitHub repository name"
  type        = string
}

# Git branch to sync values from (from GitHub Variable: GIT_BRANCH)
